...
export function CTA({ href, label }: { href: string; label: string }) {
  return (
    <>
      <Link href={href} className="relative bg-theme-1 rounded-lg px-10 py-2.5 md:pl-15 md:rounded-xl md:pr-20 md:py-3 cursor-pointer flex">
...
export function CTAFunc({ handleFunc, label }: { handleFunc: () => void; label: string }) {
  return (
    <>
      <button type="button" className="relative z-50 bg-theme-1 rounded-lg text-white px-11.75 py-2.75 text-xs leading-[1.22] tracking-0" onClick={handleFunc}>
        <p>{label}</p>
