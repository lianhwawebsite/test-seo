  ...
  return (
    <>
      <Link href={href} className="relative bg-theme-1 rounded-lg px-10 py-2.5 md:pl-15 md:rounded-xl md:pr-20 md:py-3 cursor-pointer flex active:bg-customDarkGray">
        <span className="text-white font-medium text-sm tracking-[0px] md:font-bold md:text-base md:leading-[1.32] md:tracking-[.5px]">{label}</span>
...
export function CTAFunc({ handleFunc, label }: { handleFunc: () => void; label: string }) {
  return (
    <>
      <button type="button" className="relative z-50 bg-theme-1 rounded-lg text-white px-11.75 py-2.75 text-sm leading-[1.22] tracking-0 active:bg-customDarkGray" onClick={handleFunc}>
        <p>{label}</p>
