...
export default function Search({ initialQuery = "", initialAnimals = [], initialTypes = [] }: { initialQuery?: string; initialAnimals?: string[]; initialTypes?: string[] }) {
  ...
  return (
    <>
      <form onSubmit={handleSubmit} action="/products" method="GET" role="search" className="flex flex-1 flex-shrink-0 mt-5 mb-6">
        <label htmlFor="search" className="sr-only">
          Search
        </label>
        <input className="block w-full rounded-full border py-[9px] pl-5 text-sm  placeholder:text-gray-500 focus:outline-0" placeholder="搜尋藥品名稱及編號..." onChange={(e) => setInputValue(e.target.value)} value={inputValue} />
      ...
      <div className="mb-4">
        ...
        <div className="mt-2 flex gap-2 flex-wrap">
          {allTypes.map((t, idx) => (
            <button key={idx} type="button" className={`border px-3 py-1 rounded-full ${types.includes(t) ? "bg-stone-500 text-white" : "bg-white"}`} onClick={() => toggle(t, types, setTypes)}>
              {t}
            </button>
          ))}
        </div>
      ...
      <div className="mb-6">
        ...
        <div className="mt-2 flex gap-2 flex-wrap">
          {allAnimals.map((a: string, idx) => (
            <button key={idx} type="button" className={`border px-3 py-1 rounded-full ${animals.includes(a) ? "bg-stone-500 text-white" : "bg-white"}`} onClick={() => toggle(a, animals, setAnimals)}>
              {a}
            </button>
          ))}
        </div>
