  ...
  return (
    <div className="relative -bottom-4 flex flex-col md:h-96 w-full md:grid mx-auto max-w-[1440px] gap-4 lg:gap-20 md:grid-rows-1 md:grid-cols-6 py-10 px-6 lg:pt-16 lg:pb-20 lg:px-52">
      <div className="mx-auto md:col-span-3 flex flex-col justify-center items-center gap-6 text-center md:text-left">
        <div className="text-center text-xl md:text-[26px] lg:text-[34px] font-semibold md:leading-[1.3]">
          {data.title?.split("&").map((item, index, arr) => (
            <Fragment key={index}>
              {item}
              {index !== arr.length - 1 && (
                <>
      ...
      <div className="md:col-span-3 text-sm text-start leading-[1.28] md:text-[15px] md:leading-[1.6] tracking-[0.4px] md:row-span-1 flex flex-col justify-center">
